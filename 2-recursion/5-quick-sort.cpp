...
/**
 * 快速排序的 naive 实现：借助简单荷兰国旗问题的方法二。将区间划分为 "<=" 和 ">"，每次确定一个元素的位置，然后递归。
 * */
...
/**
 * 快速排序的实现：借助荷兰国旗问题的方法二。将区间划分为 "<"、"=" 和 ">"，每次确定一组元素（中间区间）的位置，然后递归。
 * 外圈是递归，平均是 O (log n）)，内圈是国旗问题，复杂度为 O(n)，因此整体复杂度为 O (n log n)。
 * 最坏是 O (n^2)。
 * */
...
/**
 * 快速排序的标准写法 —— 等概率随机选择一个位置，而不是最后一个位置。复杂度为 O (n log n)。
 * 空间复杂度：最坏为 O(n)（栈的深度），一般为 O(log n)。
 * */

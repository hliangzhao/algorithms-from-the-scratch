...
/// 它和上述三个函数的签名一致。我们用 (*seq_func)，一个指针，来作为对应函数的名字，从而指向对应的函数。
/// 这种写法叫做「函数指针」，seq_func 就是该变量的名字。
bool find_element(int pos, double &elem, const vector<double> *(*seq_func)(int));

enum seq_types {
    type_fib, type_square, type_circle
};
...
bool find_element(int pos, double &elem, const vector<double> *(*seq_func)(int)) {
    ...
    /// seq_func(pos) 得到一个具体的函数，即 fib_seq()、square_seq()、circle_seq() 三者其一。
    /// 调用这个函数，拿到返回的 seq，因为这三个函数返回的是静态局部变量，因此 seq 需要用 const 修饰。
    const vector<double> *seq = seq_func(pos);
    elem = (*seq)[pos - 1];
    return true;
}
...
int main() {
    ...
    cout << endl;
    for (int j = 1; j < 4; j++) {
        // 只关心 square 数列的输出，用 enum 变量直接获得对应的函数指针
        if (find_element(j, elem, seq_arr[type_square])) {
            cout << elem << " ";
        }
    }
    cout << endl;
    for (int j = 1; j < 4; j++) {
        // 只关心 circle 数列的输出，用 enum 变量直接获得对应的函数指针
        if (find_element(j, elem, seq_arr[type_circle])) {
            cout << elem << " ";
        }
    }
    cout << endl;
